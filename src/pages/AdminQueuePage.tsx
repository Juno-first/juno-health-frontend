...
import CheckInTriggerModal from "../components/CheckInTriggerModal";
...
const FALLBACK_AI_ALERTS = [
  { Icon: CircleAlert,         color:"red",    label:"Critical",   age:"2m",  title:"Patient #1847 vitals deteriorating",  sub:"Immediate escalation recommended"   },
  { Icon: Clock,               color:"orange", label:"Wait Risk",  age:"5m",  title:"3 patients exceeding wait targets",   sub:"Consider staff reallocation"         },
  { Icon: ChartNoAxesCombined, color:"blue",   label:"Pattern",    age:"8m",  title:"Respiratory symptom clustering",      sub:"5 similar cases in last hour"        },
  { Icon: Lightbulb,           color:"purple", label:"Insight",    age:"12m", title:"Peak volume predicted in 45 minutes", sub:"Recommend additional triage staff"   },
  { Icon: TriangleAlert,       color:"yellow", label:"Warning",    age:"15m", title:"Radiology department at capacity",    sub:"2 hour backlog for imaging studies"  },
  { Icon: CircleCheck,         color:"green",  label:"Efficiency", age:"18m", title:"Throughput rate improved 15%",        sub:"Current performance above target"    },
  { Icon: UserRound,           color:"blue",   label:"Staffing",   age:"22m", title:"Dr. Martinez available in 10 min",   sub:"Shift change approaching at 15:00"   },
  { Icon: BedDouble,           color:"orange", label:"Capacity",   age:"25m", title:"4 beds available in critical care",  sub:"General ward at 92% occupancy"       },
];
...
const AI_INSIGHT_TYPE_CONFIG: Record<
  string,
  {
    color: keyof typeof ALERT_CFG;
    label: string;
...
const TIER_GROUPS: { tier: PriorityTier; label: string; dot: string; text: string }[] = [
  { tier:"RESUSCITATION", label:"Immediate / Resuscitation", dot:"#000000", text:"#FFFFFF" },
...
function getRelatedEntry(insight: QueueInsight, entries: AdminQueueEntry[]) {
...
function PatientDrawer({ entry, onClose, onCallClick, onDischarge, onCheckIn }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end lg:items-center justify-center fade-slide">
      ...
      <div
        className="relative w-full lg:max-w-lg max-h-[90vh] overflow-y-auto rounded-t-3xl lg:rounded-3xl border"
        style={{ background: CARD, borderColor: BORD2 }}
      >
        {/* Sticky header */}
        <div
          className="sticky top-0 px-6 pt-6 pb-4 border-b z-10"
          style={{ background: CARD, borderColor: BORDER }}
        >
          <div className="flex items-center justify-between mb-4">
            <span className={`px-3 py-1 rounded-full text-xs font-black ${pc.bg} ${pc.text}`}>
              {pc.label}
            </span>
            <button
              onClick={onClose}
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">
                ...
                {entry.age != null && " · "}
                {entry.checkinCode
                  ? <>Code <span className="font-bold tracking-widest text-gray-200">{entry.checkinCode}</span> · </>
                  : null}
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={s.label} className="text-center p-3 rounded-xl" style={{ background: BG }}>
                <div className={`text-xl font-black ${(s as { red?: boolean }).red ? "text-red-400" : "text-white"}`}>
                  {s.val}
                </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {entry.symptomCategories.map(c => (
                <span
                  key={c}
                  className="px-3 py-1 rounded-full text-xs font-semibold text-blue-300"
                  style={{
          ...
          {entry.additionalNotes && (
            <div>
              ...
              <p
                className="text-sm text-amber-300 p-4 rounded-xl leading-relaxed"
                style={{
        ...
        {/* Sticky footer — only shown when patient is still waiting */}
        {entry.status === "CHECKED_IN" && (
          <div
            className="sticky bottom-0 px-6 py-4 border-t flex flex-col gap-2"
            style={{ background: CARD, borderColor: BORDER }}
          >
            <div className="flex gap-3">
              <button
                onClick={() => { onClose(); onCallClick(entry); }}
              ...
              <button
                onClick={() => { onDischarge(entry.queueEntryId); onClose(); }}
...
function LiveInsightCard({
  ...
  const relatedEntry = getRelatedEntry(insight, entries);
  ...
  const clickable = !!relatedEntry && !!onSelectEntry;
  ...
  return (
    <div
      ...
      onClick={() => {
        if (relatedEntry && onSelectEntry) onSelectEntry(relatedEntry);
      }}
    >
      <div className="flex items-start justify-between mb-1.5 gap-2">
        ...
        <span
          ...
          style={{ color: severityAccent(insight.severity) }}
        >
          {insight.severity}
        </span>
      </div>
      ...
      <div className="flex items-center justify-between mt-2 gap-2">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {relatedEntry && (
            ...
            >
              {relatedEntry.presentingComplaint.split(",")[0].trim()}
            </span>
          )}
        </div>
...
export default function AdminQueuePage() {
  ...
  const longestWaitIso = entries.length
    ? entries.reduce((a, b) => waitMins(a.checkedInAt) > waitMins(b.checkedInAt) ? a : b, entries[0]).checkedInAt
    : new Date().toISOString();
  const grouped       = TIER_GROUPS.map(g => ({ ...g, entries: entries.filter(e => e.priorityTier === g.tier) }));

  const liveInsights = insights ?? [];
  const hasLiveInsights = liveInsights.length > 0;
  ...
  const STATS = [
    ...
    { label:"Longest Waiting",    val:formatWait(longestWaitIso), Icon:Hourglass,  iconBg:"rgba(234,179,8,.2)",   iconColor:"#FACC15", pulse:false },
  ...
  const insightSocketLabel = {
    idle: "Idle",
    connecting: "Connecting…",
    connected: "Live",
    disconnected: "Reconnecting…",
    error: "Error",
  }[insightsSocketStatus];

  const insightSocketColor =
    insightsSocketStatus === "connected"
      ? { fg: "#C084FC", bg: "rgba(168,85,247,.12)" }
      : insightsSocketStatus === "error"
      ? { fg: "#F87171", bg: "rgba(239,68,68,.1)" }
      : { fg: "#FB923C", bg: "rgba(249,115,22,.1)" };
                ...
                >
                  <div className="flex items-center justify-between gap-3 mb-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">Live queue insights & suggestions</p>
                      </div>
                    </div>
                    ...
                    <div
                      ...
                      style={{
                        color: insightSocketColor.fg,
                        background: insightSocketColor.bg,
                      }}
                    >
                      {insightsSocketStatus === "connected" ? (
                        <Wifi size={11} />
                      ) : insightsSocketStatus === "connecting" || insightsSocketStatus === "disconnected" ? (
                        <Loader2 size={11} className="animate-spin" />
                      ) : (
                        <WifiOff size={11} />
                      )}
                      <span>{insightSocketLabel}</span>
                    </div>
                  </div>
                    ...
                    >
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <p className="text-xs font-bold text-white truncate">
                          {insightsPayload.departmentName}
                        </p>
                        <span className="text-[10px] text-gray-500 flex-shrink-0">
                          {insightsPayload.queueDepth != null && (
                            <span className="text-[10px] text-gray-500 flex-shrink-0">
                              {insightsPayload.queueDepth} in queue
                            </span>
                          )}
                        </span>
                      </div>
                      <p className="text-[11px] text-gray-400">
                        Triggered by {insightsPayload.eventType?.toLowerCase().replaceAll("_", " ") ?? "patient check"}
                      </p>
                    </div>
                  )}
                  ...
                  <div className="space-y-3">
                    {hasLiveInsights ? (
                      ...
                      FALLBACK_AI_ALERTS.map((a, i) => {
                        const cfg = ALERT_CFG[a.color] ?? ALERT_CFG.blue;
                        return (
                          <div key={i} className="rounded-xl p-3 border" style={{ background:cfg.bg, borderColor:cfg.border }}>
                            <div className="flex items-start justify-between mb-1.5">
                              <div className="flex items-center gap-1.5">
                                <a.Icon size={12} style={{ color:cfg.text }} />
                                <span className="text-xs font-black uppercase tracking-wider" style={{ color:cfg.text }}>{a.label}</span>
                              </div>
                              <span className="text-xs text-gray-500">{a.age}</span>
                            </div>
                            <p className="text-xs text-white font-semibold mb-0.5">{a.title}</p>
                  ...
                  <button className="w-full mt-4 py-2.5 rounded-xl text-white text-sm font-bold transition-all hover:brightness-110"
